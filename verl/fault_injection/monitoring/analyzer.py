...
class FaultImpactAnalyzer:
    ...
    def _initialize_default_dependencies(self) -> None:
        ...
        self.add_dependency("actor_system", layer=FaultLayer.ORCHESTRATION, depends_on=["ray_cluster"], criticality="high")
        self.add_dependency("resource_pool", layer=FaultLayer.ORCHESTRATION, depends_on=["actor_system"], criticality="medium")

        # Worker Layer dependencies
        self.add_dependency("fsdp_workers", layer=FaultLayer.WORKER, depends_on=["actor_system", "resource_pool"], criticality="high")
        self.add_dependency("megatron_workers", layer=FaultLayer.WORKER, depends_on=["actor_system", "resource_pool"], criticality="high")
        self.add_dependency("gradient_sync", layer=FaultLayer.WORKER, depends_on=["fsdp_workers", "megatron_workers"], criticality="high")

        # Engine Layer dependencies
        self.add_dependency("training_engine", layer=FaultLayer.ENGINE, depends_on=["fsdp_workers", "megatron_workers"], criticality="critical")
        self.add_dependency("checkpoint_system", layer=FaultLayer.ENGINE, depends_on=["training_engine"], criticality="high")
        ...
        self.add_dependency("vllm_backend", layer=FaultLayer.INFERENCE, depends_on=["training_engine"], criticality="medium")
        self.add_dependency("sglang_backend", layer=FaultLayer.INFERENCE, depends_on=["training_engine"], criticality="medium")
        self.add_dependency("kv_cache", layer=FaultLayer.INFERENCE, depends_on=["vllm_backend", "sglang_backend"], criticality="medium")
    ...
    def add_dependency(self, component: str, depends_on: Optional[List[str]] = None,
                        dependents: Optional[List[str]] = None, layer: Optional[FaultLayer] = None,
    ...
    def analyze_fault_impact(self, fault_result: FaultResult,
                           historical_window_hours: int = 24) -> ImpactAnalysisResult:
        ...
        impact_metrics = self._calculate_impact_metrics(
            fault_result, affected_components, historical_window_hours
        )
    ...
    def _get_affected_components(self, fault_result: FaultResult) -> Set[str]:
        ...
        if layer:
            layer_components = {
                c for c, dep in self._dependencies.items()
                if dep.layer == layer
            }
    ...
    def _calculate_impact_metrics(self, fault_result: FaultResult,
                                affected_components: Set[str],
                                historical_window_hours: int) -> ImpactMetrics:
        ...
        metrics.affected_layers = {
            self._dependencies[c].layer for c in affected_components
        ...
        # Cascade probability
        metrics.cascade_probability = self._calculate_cascade_probability(
            fault_result, affected_components
        )

        # System degradation
        metrics.system_degradation = self._calculate_system_degradation(
            fault_result, affected_components
        )

        return metrics

    def _calculate_cascade_probability(self, fault_result: FaultResult,
                                     affected_components: Set[str]) -> float:
    ...
    def _calculate_system_degradation(self, fault_result: FaultResult,
                                    affected_components: Set[str]) -> float:
    ...
    def _assess_cascade_risk(self, fault_result: FaultResult,
                           affected_components: Set[str]) -> str:
    ...
    def _generate_recommendations(self, fault_result: FaultResult,
        ...
        critical_components = {
            c for c in impact_metrics.affected_components
            if c in self._dependencies
            and self._dependencies[c].criticality == "critical"
        }
    ...
    def _calculate_confidence(self, fault_result: FaultResult,
                            historical_window_hours: int) -> float:
        ...
        recent_faults = [
            f for f in self._historical_faults
